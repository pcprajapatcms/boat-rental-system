<?php
/**
 * Reservation cancellation request declined email class.
 *
 ...
 * @extends \WC_Email
 */
class Reservation_Cancellation_Request_declined_Email extends WC_Email {
	/**
	 * Set email defaults.
	 *
	 * @since 1.0.0
	...
	public function __construct() {
		...
		$this->id          = 'reservation_cancellation_request_declined_email';
		$this->title       = __( 'Easy Reservations: Reservation Cancellation Request Declined Email', 'easy-reservations' );
		$this->description = __( 'An email sent to the customer when admin declines reservation cancellation request.', 'easy-reservations' );

		// For admin area to let the user know we are sending this email to the customer.
		$this->customer_email = true;
		$this->heading        = __( 'Reservation Cancellation Request Declined', 'easy-reservations' );

		// translators: placeholder is {blogname}, a variable that will be substituted when email is sent out.
		$this->subject = sprintf( _x( '[%s] Reservation Cancellation Request Declined', 'default email subject for reservation cancellation request declined being sent to the customer', 'easy-reservations' ), '{blogname}' );

		// Template paths.
		$this->template_html  = 'reservation-cancellation-request-declined-html.php';
		$this->template_plain = 'plain/reservation-cancellation-request-declined-plain.php';

		add_action( 'ersrv_send_reservation_cancellation_request_declined_notification', array( $this, 'ersrv_ersrv_send_reservation_cancellation_request_declined_notification_callback' ) );
	 ...
	 * @since 1.0.0
	 */
	public function ersrv_ersrv_send_reservation_cancellation_request_declined_notification_callback( $line_item_id ) {
